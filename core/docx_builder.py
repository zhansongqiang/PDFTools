"""Word 文档生成模块 - 将解析后的内容生成 .docx 文件
Copyright (c) 2026 zsq. All rights reserved.
版本: 1.0
联系邮箱: 11016795@qq.com
联系电话: 18820818283

改进：
- 标题使用 add_heading()（Word 导航和目录可用）
- PDF 字体 → Word 字体映射
- 表格智能对齐
- 图片尺寸修正
"""
from __future__ import annotations

import io
from pathlib import Path

from docx import Document
from docx.shared import Pt, Inches, Cm
from docx.enum.text import WD_ALIGN_PARAGRAPH
from docx.enum.table import WD_TABLE_ALIGNMENT
from docx.oxml.ns import qn, nsdecls
from docx.oxml import parse_xml, OxmlElement

from .pdf_parser import PageContent, TextBlock, ImageBlock, TableBlock


# ==================== 字体映射 ====================

# PDF 字体名 → Word 字体名
_FONT_MAP = {
    "simsun": "宋体",
    "宋体": "宋体",
    "songti": "宋体",
    "simhei": "黑体",
    "黑体": "黑体",
    "heiti": "黑体",
    "kaiti": "楷体",
    "楷体": "楷体",
    "fangsong": "仿宋",
    "仿宋": "仿宋",
    "microsoftyahei": "微软雅黑",
    "微软雅黑": "微软雅黑",
    "yahei": "微软雅黑",
    "arial": "Arial",
    "timesnewroman": "Times New Roman",
    "times": "Times New Roman",
    "courier": "Courier New",
    "helvetica": "Arial",
}


def _map_font(pdf_font_name: str) -> str:
    """将 PDF 字体名映射为 Word 字体名"""
    if not pdf_font_name:
        return "宋体"
    lower = pdf_font_name.lower().replace(" ", "").replace("-", "")
    for key, value in _FONT_MAP.items():
        if key in lower:
            return value
    # 检查原始名是否含中文
    for ch in pdf_font_name:
        if '\u4e00' <= ch <= '\u9fff':
            return pdf_font_name  # 可能本身就是正确的中文字体名
    return "宋体"


def _is_numeric(text: str) -> bool:
    """判断文本是否为数字"""
    try:
        float(text.strip().replace(",", "").replace("%", ""))
        return True
    except ValueError:
        return False


# ==================== 样式设置 ====================

def _setup_default_style(doc: Document):
    """设置文档默认样式"""
    style = doc.styles["Normal"]
    font = style.font
    font.name = "宋体"
    font.size = Pt(12)
    style.element.rPr.rFonts.set(qn('w:eastAsia'), '宋体')

    pf = style.paragraph_format
    pf.space_before = Pt(0)
    pf.space_after = Pt(0)
    pf.line_spacing = 1.5

    # 配置标题样式
    for level in range(1, 4):
        heading_style = doc.styles[f"Heading {level}"]
        hf = heading_style.font
        hf.name = "黑体"
        heading_style.element.rPr.rFonts.set(qn('w:eastAsia'), '黑体')
        hf.bold = True
        heading_style.paragraph_format.line_spacing = 1.5

    # Heading 1
    h1 = doc.styles["Heading 1"]
    h1.font.size = Pt(18)
    h1.paragraph_format.space_before = Pt(18)
    h1.paragraph_format.space_after = Pt(6)

    # Heading 2
    h2 = doc.styles["Heading 2"]
    h2.font.size = Pt(16)
    h2.paragraph_format.space_before = Pt(12)
    h2.paragraph_format.space_after = Pt(6)

    # Heading 3
    h3 = doc.styles["Heading 3"]
    h3.font.size = Pt(14)
    h3.paragraph_format.space_before = Pt(6)
    h3.paragraph_format.space_after = Pt(3)


def _set_run_font(run, font_name: str = "宋体"):
    """设置 run 的字体"""
    run.font.name = font_name
    run.element.rPr.rFonts.set(qn('w:eastAsia'), font_name)


# ==================== 主函数 ====================

def build_docx(
    pages: list[PageContent],
    output_path: str | Path,
    progress_callback=None,
) -> str:
    """将页面内容生成 Word 文档"""
    doc = Document()
    total_pages = len(pages)

    _setup_default_style(doc)

    # 第一遍：收集所有页面元素
    all_elements = []
    for page_idx, page in enumerate(pages):
        if progress_callback:
            progress_callback(page_idx + 1, total_pages)

        elements = []
        for tb in page.text_blocks:
            elements.append(("text", tb.y, tb))
        for ib in page.image_blocks:
            elements.append(("image", ib.y, ib))
        for tbl in page.table_blocks:
            elements.append(("table", tbl.y, tbl))

        elements.sort(key=lambda e: e[1])
        all_elements.append((page_idx, elements))

    # 第二遍：生成文档
    for page_idx, elements in all_elements:
        for elem_type, _, elem in elements:
            if elem_type == "text":
                _add_text_block(doc, elem)
            elif elem_type == "image":
                _add_image_block(doc, elem)
            elif elem_type == "table":
                _add_table_block(doc, elem)

        # 页面间分页符（最后一页除外）
        if page_idx < total_pages - 1:
            doc.add_page_break()

    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    doc.save(str(output_path))
    return str(output_path)


# ==================== 文本块处理 ====================

def _add_text_block(doc: Document, block: TextBlock):
    """添加文本块"""
    if block.is_title:
        _add_title_block(doc, block)
    elif block.is_toc_entry:
        para = doc.add_paragraph()
        _add_toc_block(doc, para, block)
    else:
        para = doc.add_paragraph()
        _add_body_block(doc, para, block)


def _add_title_block(doc: Document, block: TextBlock):
    """添加标题 — 使用 add_heading() 使 Word 导航可用"""
    # 封面大标题
    if block.title_level == 1 and block.font_size >= 20:
        para = doc.add_heading(level=0)  # Title level
        para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = para.add_run(block.text)
        run.font.size = Pt(min(block.font_size, 28))
        run.font.bold = True
        _set_run_font(run, "黑体")
        return

    # 一级标题
    if block.title_level == 1:
        para = doc.add_heading(level=1)
        run = para.add_run(block.text)
        run.font.size = Pt(min(block.font_size, 18))
        run.font.bold = True
        _set_run_font(run, "黑体")
        if block.alignment == "center":
            para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        return

    # 二级标题
    if block.title_level == 2:
        para = doc.add_heading(level=2)
        run = para.add_run(block.text)
        run.font.size = Pt(min(block.font_size, 16))
        run.font.bold = True
        _set_run_font(run, "黑体")
        return

    # 三级标题
    if block.title_level == 3:
        para = doc.add_heading(level=3)
        run = para.add_run(block.text)
        run.font.size = Pt(min(block.font_size, 14))
        run.font.bold = True
        _set_run_font(run, "黑体")
        return


def _add_toc_block(doc: Document, para, block: TextBlock):
    """添加目录条目"""
    pf = para.paragraph_format
    pf.space_before = Pt(1)
    pf.space_after = Pt(1)
    pf.line_spacing = 1.15
    para.alignment = WD_ALIGN_PARAGRAPH.LEFT

    text = block.text
    parts = text.split('\t')
    if len(parts) >= 2:
        title_part = parts[0].strip()
        page_part = parts[-1].strip()
        run = para.add_run(title_part)
        run.font.size = Pt(10.5)
        _set_run_font(run, "宋体")
        run = para.add_run("\t")
        run.font.size = Pt(10.5)
        run = para.add_run(page_part)
        run.font.size = Pt(10.5)
        _set_run_font(run, "宋体")
    else:
        run = para.add_run(text)
        run.font.size = Pt(10.5)
        _set_run_font(run, "宋体")

    _add_tab_stop(para)


def _add_tab_stop(para):
    """添加右对齐制表位（带点线引导符）"""
    pPr = para._element.get_or_add_pPr()
    tabs = OxmlElement('w:tabs')
    tab = OxmlElement('w:tab')
    tab.set(qn('w:val'), 'right')
    tab.set(qn('w:pos'), '9000')
    tab.set(qn('w:leader'), 'dot')
    tabs.append(tab)
    pPr.append(tabs)


def _add_body_block(doc: Document, para, block: TextBlock):
    """添加正文段落"""
    pf = para.paragraph_format
    pf.space_before = Pt(0)
    pf.space_after = Pt(0)
    pf.line_spacing = 1.5

    if block.first_line_indent:
        pf.first_line_indent = Cm(0.74)

    # 图表标题（小字号，居中）
    if block.alignment == "center" and not block.first_line_indent:
        para.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = para.add_run(block.text)
        run.font.size = Pt(block.font_size) if block.font_size > 0 else Pt(10.5)
        _set_run_font(run, _map_font(block.font_name))
        return

    # 普通正文 - 保留混合格式
    if block.spans_info:
        for span_info in block.spans_info:
            run = para.add_run(span_info["text"])
            size = span_info["size"]
            if size > 0:
                run.font.size = Pt(size)
            if span_info["bold"]:
                run.font.bold = True
            if span_info["italic"]:
                run.font.italic = True
            span_font = _map_font(span_info.get("font", ""))
            _set_run_font(run, span_font)
    else:
        run = para.add_run(block.text)
        if block.font_size > 0:
            run.font.size = Pt(block.font_size)
        if block.bold:
            run.font.bold = True
        if block.italic:
            run.font.italic = True
        _set_run_font(run, _map_font(block.font_name))


# ==================== 图片处理 ====================

def _add_image_block(doc: Document, block: ImageBlock):
    """添加图片"""
    if block.ocr_text:
        # OCR 文字
        first_line = True
        for line in block.ocr_text.split("\n"):
            if line.strip():
                para = doc.add_paragraph()
                pf = para.paragraph_format
                if first_line:
                    pf.first_line_indent = Cm(0.74)
                    first_line = False
                run = para.add_run(line)
                run.font.size = Pt(12)
                _set_run_font(run, "宋体")
    else:
        try:
            image_stream = io.BytesIO(block.data)
            width = None
            if block.width > 0:
                max_width = 6.0
                calc_width = block.width / 72
                width = Inches(min(calc_width, max_width))
            para = doc.add_paragraph()
            para.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = para.add_run()
            run.add_picture(image_stream, width=width)
        except Exception:
            doc.add_paragraph("[图片无法显示]")


# ==================== 表格处理 ====================

def _add_table_block(doc: Document, block: TableBlock):
    """添加表格 — 智能对齐"""
    if not block.rows:
        return

    num_cols = max(len(row) for row in block.rows)
    if num_cols == 0:
        return

    table = doc.add_table(rows=len(block.rows), cols=num_cols)
    table.style = "Table Grid"
    table.alignment = WD_TABLE_ALIGNMENT.CENTER

    # 表格宽度 100%
    tbl = table._tbl
    tblPr = tbl.tblPr if tbl.tblPr is not None else parse_xml(f'<w:tblPr {nsdecls("w")}/>')
    tblW = parse_xml(f'<w:tblW {nsdecls("w")} w:w="5000" w:type="pct"/>')
    tblPr.append(tblW)

    for row_idx, row in enumerate(block.rows):
        for col_idx, cell in enumerate(row):
            if col_idx >= num_cols:
                continue
            table_cell = table.cell(row_idx, col_idx)
            for p in table_cell.paragraphs:
                p.clear()

            if table_cell.paragraphs:
                para = table_cell.paragraphs[0]
            else:
                para = table_cell.add_paragraph()

            # 智能对齐：表头居中，数值右对齐，文本左对齐
            cell_text = cell.text
            if row_idx == 0:
                para.alignment = WD_ALIGN_PARAGRAPH.CENTER
            elif _is_numeric(cell_text):
                para.alignment = WD_ALIGN_PARAGRAPH.RIGHT
            else:
                para.alignment = WD_ALIGN_PARAGRAPH.LEFT

            run = para.add_run(cell_text)
            run.font.size = Pt(10.5)
            _set_run_font(run, "宋体")

            if row_idx == 0 or cell.bold:
                run.font.bold = True

            # 垂直居中
            tc = table_cell._tc
            tcPr = tc.get_or_add_tcPr()
            vAlign = parse_xml(f'<w:vAlign {nsdecls("w")} w:val="center"/>')
            tcPr.append(vAlign)
